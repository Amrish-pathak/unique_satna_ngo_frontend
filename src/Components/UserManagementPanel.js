...
const UserManagementPanel = () => {
  ...
  const handleInputChange = (e) => {
    ...
    setEditUserData({
      ...editUserData,
      [name]: name === 'balance' || name === 'spins' ? Number(value) : value
    });
  };
  ...
  return (
    <div className="w-full flex flex-col space-y-4 h-[100vh] scroller pt-4 overflow-y-auto pb-[150px]">
      <div className='w-full sm:w-[50%] flex flex-col gap-3'>
        ...
        {user && (
          <div className='w-full flex flex-col space-y-3 bg-[#dab1b1] p-4 rounded-[10px] text-[13px]'>
                ...
                <div className="flex w-full flex-wrap gap-3">
                  ...
                  <div className='flex flex-col w-full gap-1'>
                    ...
                    <input
                      type="number"
                      name="spins"
                      value={editUserData.spins}
                      onChange={handleInputChange}
                      placeholder="Spin Balance"
                      className="bg-[#4b4b4b] placeholder:text-[#b9b9b9] text-[#e0e0e0] placeholder:text-[12px] text-[13px] placeholder:font-light h-[55px] border-none outline-none rounded-[10px] flex items-center px-6"
                    />
                  </div>
                </div>
      ...
      <div className="w-full sm:w-[50%] flex flex-col space-y-3">
        ...
        {users.map((user, index) => (
          <div key={user.id} className="bg-[#56489694] p-4 rounded-[10px] text-[13px] relative flex flex-col w-full space-y-2">
            ...
            <div className='flex gap-5'>
              ...
              <span className='flex items-center gap-1 psl-1'>
                <img src='/trophy.webp' alt="balance" className="w-[14px] h-[14px] rounded-full" />
                <p><span className='font-semibold text-accent'> {formatNumber(user.spins)}</span></p>
              </span>
            ...
            {dropdownVisible[user.id] && (
              <div className="absolute z-10 top-8 right-2 bg-[#8a56bb] text-primary rounded-md shadow-lg w-40">
                <button
                  onClick={() => {
                    setUser(user);
